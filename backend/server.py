...
async def register_user(
    user_data: UserCreate, current_user: Optional[User] = Depends(get_current_user)
):
...
async def get_certificate(
    certificate_id: str, current_user: User = Depends(get_current_user)
):
    ...
    if (
        ...
        relationship = await db.relationships.find_one(
            {"distributor_id": current_user.id, "retailer_id": cert_obj.retailer_id}
        )
        if not relationship:
            raise HTTPException(status_code=403, detail="Access forbidden")
...
async def upload_certificate_image(
    ...
    await db.certificates.update_one(
        ...
        {
            "$set": {
                f"images.{image_type}": encoded_image,
...
async def get_dashboard_stats(current_user: User = Depends(get_current_user)):
    if current_user.role == UserRole.ADMIN:
        ...
        total_distributors = await db.users.count_documents(
        ...
        submitted_certificates = await db.certificates.count_documents(
            {"retailer_id": {"$in": retailer_ids}, "status": "submitted"}
        )
        ...
        submitted_certificates = await db.certificates.count_documents(
            {"retailer_id": current_user.id, "status": "submitted"}
        )
